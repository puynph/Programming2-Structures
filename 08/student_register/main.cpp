...
void saveDataToFile (std::map<std::string, Student*> const &users, std::string const &fileName)
{
    std::ofstream file(fileName);
    for (auto const &user : users)
    {
        Student* p = user.second;
        std::string line = p->student_number + ";"
                          +p->user_id + ";"
                          +p->name + ";"
                          +p->phone_number + ";"
                          +p->email + ";"
...
int main() {
    ...
    while(true) {
        ...
        if(command == "U" or command == "u") {
            ...
            // fetch student and modify
            student_numbers.at(parts.at(1))->phone_number = phone;
