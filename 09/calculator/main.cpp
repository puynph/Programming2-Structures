...
const string GREETING_AT_END = "Thanks and see you later!",
             ERROR_NON_NUMBER = "Error: a non-number operand.",
             ERROR_WRONG_PARAM_AMOUNT = "Error: wrong number of parameters.",
             UNKOWN_COMMAND = "Error: unknown command.";
...
void executeCalculation(double(*f)(double, double), std::string param1, std::string param2)
{
...
int main() {
    ...
    while ( true ) {
        ...
        // find matching command to command_to_be_executed
        for (Command const &comm : COMMANDS)
        {
            ...
            {
                commandFound = true;
                // check the amount of command parameter
                if (pieces.size() - 1 == comm.parameter_number)
                {
                    ...
                    executeCalculation(comm.action, pieces.at(1), pieces.at(2));

                }
                else
                {
                    std::cout << ERROR_WRONG_PARAM_AMOUNT << std::endl;
                }
            }
        }
        if (!commandFound)
        {
            std::cout << UNKOWN_COMMAND << std::endl;
        }
