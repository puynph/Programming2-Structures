...
{
    for( std::vector< int >::size_type i = 0; i < ints.size() - 1; i++ )
    {
        if (ints[i] != ints[i+1])
        {
            return false;
...
bool is_arithmetic_series(std::vector< int >& ints)
{
    ...
    for( std::vector< int >::size_type i = 1; i < ints.size() - 1; i++ )
    {
